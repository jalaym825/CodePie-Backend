...
router.put("/callback", async (req, res) => {
  const { stdout, time, memory, stderr, compile_output, message, status } =
    req.body;
  const { userId, isSubmission, problemId, testCaseId, submissionId } =
    req.query;
  ...
  if (isSubmission === "true") {
    try {
      ...
      if (processedTestCases === allTestCases.length) {
        ...
        let overallStatus;
        const allPassed = submission.testCaseResults.every(
        ...
        if (allPassed) {
          ...
          submission.testCaseResults.some(
            (r) => r.status === "COMPILATION_ERROR"
          )
        ) {
          overallStatus = "COMPILATION_ERROR";
        } else if (
          submission.testCaseResults.some((r) => r.status === "RUNTIME_ERROR")
        ) {
        ...
        await prisma.submission.update({
          ...
          data: {
            ...
            executionTime: Math.max(
              ...submission.testCaseResults.map((r) => r.executionTime || 0)
            ),
            memoryUsed: Math.max(
              ...submission.testCaseResults.map((r) => r.memoryUsed || 0)
            ),
        ...
        if (problem && problem.contest) {
          ...
          if (participation) {
            ...
            for (const sub of userSubmissions) {
              if (
                !problemScores[sub.problemId] ||
                sub.score > problemScores[sub.problemId]
              ) {
            ...
            const totalParticipationScore = Object.values(problemScores).reduce(
              (sum, score) => sum + score,
              0
            );
